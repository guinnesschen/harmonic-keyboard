    ...
    <div className="flex items-center justify-between gap-4">
      ...
      <Select value={value} onValueChange={onChange}>
        ...
        <SelectContent>
          ...
          <SelectItem value={ChordQuality.MinAdd9}>Min Add 9</SelectItem>
        </SelectContent>
      </Select>
    </div>
  );
...
export default function SettingsModal() {
  ...
  const [localChordQualities, setLocalChordQualities] =
    useState<ChordQualityConfig>(settings.chordDefaults);
  ...
  const updateChordQuality = (
    ...
    updateSettings({ chordDefaults: newConfig });
  };
  ...
  const resetToDefaults = () => {
    ...
    updateSettings({ chordDefaults: defaultChordQualities });
  };

  const localMappings = useMemo(() => settings.keyMappings, [settings.keyMappings]);
